(() => {
    ...
    function initSliders() {
        if (document.querySelector(".block-1__slider")) new core(".block-1__slider", {
    ...
    const findTarget = e => {
        let target = e.target;
        while (null !== target) {
            if (target.classList && target.classList.contains(SCROLLABLE_CLASSNAME)) break;
    ...
    const touchstartEventHandler = (e, fixScroll) => {
        ...
        if (target) {
            ...
            if (height === totalScroll) target.dataset[PREVENT_SCROLL_DATASET] = "true";
        }
    };
    const touchmoveEventHandler = (e, fixScroll) => {
        if (!fixScroll.getState()) {
            const target = findTarget(e);
            if (target) if ("true" === target.dataset[PREVENT_SCROLL_DATASET]) e.preventDefault(); else {
                ...
                if (scrollTop <= 0) {
                    if (delta < currentDelta) e.preventDefault();
                } else if (currentScroll >= totalScroll) if (delta > currentDelta) e.preventDefault();
            }
        }
    };
    const touchendEventHandler = (e, fixScroll) => {
        const target = findTarget(e);
        if (target) target.dataset[PREVENT_SCROLL_DATASET] = "false";
    };
    ...
    class FixScroll {
        ...
        toggle() {
            if (this.getState()) this.hide(); else this.show();
        }
        getWidth() {
